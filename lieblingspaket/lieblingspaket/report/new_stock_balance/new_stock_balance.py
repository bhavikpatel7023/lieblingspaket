	...
	for (company, item, warehouse) in sorted(iwb_map):
		if item_map.get(item):
			...
			report_data = [item, item_map[item]["item_name"],
				...
				((qty_dict.bal_val / (item_map[item]["pieces_per_carton"] if item_map[item]["pieces_per_carton"] > 0 else 1)) / (item_map[item]["volume"] if item_map[item]["volume"] > 0 else 1)),
				((qty_dict.bal_val / (item_map[item]["pieces_per_carton"] if item_map[item]["pieces_per_carton"] > 0 else 1)) / (item_map[item]["weight_in_kg"] if item_map[item]["weight_in_kg"] > 0 else 1)),
